...
from app.services.storage import get_storage_response
...
def share(data: ShareCreate, user = Depends(current_user), db: Session = Depends(get_db)):
    ...
    if not f or not target:
        raise HTTPException(404, 'File or user not found')
    ...
    if s:
        ...
        db.add(Share(file_id=f.id, user_id=target.id, role=data.role))
    db.commit()
    return {'ok': True, 'email': target.email, 'role': data.role}
...
def public_link(data: LinkCreate, user = Depends(current_user), db: Session = Depends(get_db)):
    ...
    db.commit()
    db.refresh(l)
    return {'token': l.token, 'expires_at': l.expires_at}

@router.get('/public/{token}')
async def public_file(token: str, db: Session = Depends(get_db)):
    l = db.query(LinkShare).filter_by(token=token).first()
    if not l or (l.expires_at and l.expires_at < datetime.now(timezone.utc)):
